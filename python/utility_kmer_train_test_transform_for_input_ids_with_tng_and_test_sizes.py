...
def generate_train_and_test_datasets_from_input_ids(tng_file_path, val_file_path, kmer_length):
    df_train, vocab_size_tng, num_of_classes_tng = _retrieve_tsv_file(tng_file_path, kmer_length)
    df_val, vocab_size_val, num_of_classes_val = _retrieve_tsv_file(val_file_path, kmer_length)
    max_length = _get_max_token_length(df_train.input_ids)
    ...
    return dataset_train, dataset_val, max_length, vocab_size_tng, num_of_classes_tng

def _retrieve_tsv_file(file_path, kmer_length, sep='\t', header=0):
